...
export function parseWhen(
	...
	const fail = (message: string): WhenParseError => ({
		message,
		position: pos,
	});
	...
	const parseAtom = (depth: number): KeybindingWhenNode | WhenParseError => {
	...
	const parseAnd = (depth: number): KeybindingWhenNode | WhenParseError => {
	...
	const parseOr = (depth: number): KeybindingWhenNode | WhenParseError => {
