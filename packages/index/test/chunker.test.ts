...
describe("treesitter chunker — typescript", () => {
  ...
  it("extracts a class plus its methods as nested symbols", async () => {
    ...
    const cls = result.symbols.find((s) => s.name === "Counter" && s.kind === "class");
    expect(cls?.exported).toBe(true);
    const inc = result.symbols.find((s) => s.name === "inc");
    expect(inc).toBeDefined();
    expect(inc?.kind).toBe("method");
    expect(inc?.parentIndex).not.toBeNull();
    const methodChunk = result.chunks.find((c) => c.symbolName?.endsWith(".inc"));
  ...
  it("extracts interface + type declarations", async () => {
    ...
    expect(result.symbols.find((s) => s.name === "Point" && s.kind === "interface")).toBeDefined();
    expect(result.symbols.find((s) => s.name === "Vector" && s.kind === "type")).toBeDefined();
  });
