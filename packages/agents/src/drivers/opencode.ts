 ...
 */
export interface OpencodeSessionHandle extends ProviderSessionHandle {
...
const spawnOpencodeServer = (
	...
	findFreePort().then(
		(port) =>
			new Promise<OpencodeServerProcess>((resolve, reject) => {
				...
				const onStdout = (chunk: string): void => {
					stdoutBuf += chunk;
					if (OPENCODE_DEBUG)
						process.stderr.write(`[opencode.stdout] ${chunk}`);
					if (settled) return;
