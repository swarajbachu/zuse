...
describe("model matcher", () => {
	...
	it("resolves exact matches", () => {
		expect(
			createModelMatcher(table)("claude-opus-4-8")?.input_cost_per_token,
		).toBe(5e-6);
	});
	...
	it("resolves provider-prefixed matches", () => {
		...
		expect(createModelMatcher(prefixed)("claude-x")?.input_cost_per_token).toBe(
			9e-6,
		);
	});
	...
	it("prefers the closest-length fuzzy match", () => {
		expect(
			createModelMatcher(table)("gpt-5.4-mini")?.input_cost_per_token,
		).toBe(2.5e-6);
	});
...
describe("cost from pricing", () => {
	it("prices each token type independently", () => {
		...
		expect(cost).toBeCloseTo(
			1000 * 5e-6 + 500 * 25e-6 + 2000 * 6.25e-6 + 10_000 * 5e-7,
		);
	});
	...
	it("applies the fast multiplier", () => {
		...
		expect(costFromPricing(counts, pricing, true)).toBeCloseTo(
			costFromPricing(counts, pricing, false) * 2,
		);
	});
});
