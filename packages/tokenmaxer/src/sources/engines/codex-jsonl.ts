...
const parseSessionFile = async (
	...
	const sessionId = relative(sessionsDir, file)
		.split(sep)
		.join("/")
		.replace(/\.jsonl$/i, "");
	...
	try {
		for await (const line of readLines(file)) {
			...
			if (raw === null && totalUsage !== null)
				raw = subtract(totalUsage, previousTotals);
...
export const readCodexJsonl = async (input: {
	...
	for (const sessionsDir of input.sessionsDirs) {
		...
		for (const file of dirFiles) {
			...
			for (const event of await parseSessionFile(sessionsDir, file)) {
				const key = eventKey(event);
				if (seen.has(key)) continue;
				seen.add(key);
				const inputExclusive = Math.max(
