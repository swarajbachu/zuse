...
const bucketKey = (record: UsageRecord, bucket: UsageBucket, timezone: string): string => {
  if (bucket === "session") {
...
const groupRecords = (
  ...
  labelFor: (record: UsageRecord, key: string) => string = (_record, key) => key,
  ...
  return Array.from(buckets.entries())
    .map(([key, rows]) => {
      const sorted = rows.slice().sort((a, b) => a.startedAt.getTime() - b.startedAt.getTime());
      return {
        key,
...
const matchesAnyRoot = (path: string | null, roots: ReadonlyArray<string>): boolean => {
...
export const filterUsageRecords = (
  ...
  return records.filter((record) => {
    ...
    if (filters.since !== undefined && record.endedAt < filters.since) return false;
    if (filters.until !== undefined && record.startedAt > filters.until) return false;
    if (roots !== null && !matchesAnyRoot(record.projectPath, roots) && !matchesAnyRoot(record.workspacePath, roots)) {
      return false;
    }
    if (filters.includePossibleDuplicates !== true && record.possibleDuplicate) {
      return false;
    }
    return true;
  });
...
export const buildUsageReport = (input: {
  ...
  const timezone = filters.timezone ?? Intl.DateTimeFormat().resolvedOptions().timeZone ?? "UTC";
  ...
  return {
    ...
    bySource: groupRecords(records, (record) => record.sourceId, (record) => record.sourceLabel),
    byModel: groupRecords(records, (record) => record.model),
    bySession: groupRecords(
      records,
