...
The next review pass adds a post-fetch cleanliness check before origin/main branch creation, a five-minute stack-submit timeout, strict boolean validation for stack flags, and localized GitHub error messages. Watchers only prepare and send for the selected chat/environment; active repairs are tracked by generation so stop/resume cannot stall behind an older operation. Native plan feedback uses the same annotation serializer as regular server sends. The earlier watcher PR's direct manual-send flow retains the message identity and payload after ambiguous delivery; the final UX continues to stage context instead of sending it.
