...
export const routeCloudBillingRequest = (
	request: Request,
): Effect.Effect<Response | null, RelayError, CloudBillingRouteContext> =>
	Effect.gen(function* () {
		...
		if (method === "POST" && url.pathname === "/v1/cloud/billing/webhook/e2b") {
			const secret = (yield* RelayConfiguration).e2bWebhookSecret;
		...
		if (
			url.pathname !== RelayPaths.cloudBillingSummary &&
			url.pathname !== RelayPaths.cloudBillingUsage &&
			url.pathname !== RelayPaths.cloudBillingCap
		)
			return null;
		...
		if (method === "GET" && url.pathname === RelayPaths.cloudBillingSummary)
			return json(yield* billingStore.summary(period));
		if (method === "GET" && url.pathname === RelayPaths.cloudBillingUsage) {
		...
		if (method === "POST" && url.pathname === RelayPaths.cloudBillingCap) {
