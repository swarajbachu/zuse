...
const enrollRequest = (input: {
	...
	new Request(`${API_ISSUER}/v1/machines/enroll`, {
		method: "POST",
		headers: {
...
const bootStatusRequest = (
	...
	new Request(`${API_ISSUER}/v1/machines/machine_1/boot-status`, {
		method: "POST",
		headers: {
			authorization: `Bearer ${TOKEN}`,
			"content-type": "application/json",
		...
		const result = await Effect.runPromise(
			Effect.gen(function* () {
				...
				return {
					...
					environment: yield* apiStore.getEnvironment(ENVIRONMENT_ID),
				};
	...
	test("does not replace an existing same-account environment identity", async () => {
		...
		const result = await Effect.runPromise(
			Effect.gen(function* () {
				yield* seedEnrollment(Date.now());
				const apiStore = yield* ApiStore;
				yield* apiStore.registerEnvironment(
					{
						environmentId: ENVIRONMENT_ID,
						accountId: "user_a",
