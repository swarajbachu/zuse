...
Thin control-plane api for the account-based device-discovery model. It links a
...
environment traffic goes directly phone ↔ laptop. For Zuse Cloud, API also
...
## Layout
- `src/config.ts` — `ApiConfiguration` service (issuer, WorkOS JWKS, Ed25519 mint keypair, TTLs).
- `src/store.ts` — `ApiStore` service: `ApiStorePg` (prod) + `ApiStoreMemory` (tests).
...
- `src/index.ts` — `makeApi(layer)` → a `fetch` handler.
...
| `DELETE /v1/account` | WorkOS bearer | delete api data, tunnels, devices, and the identity account |
...
bun test
```
Tests wire `ApiStoreMemory` + `WorkosVerifierTest` and simulate the desktop
...
1. **Mint the api Ed25519 keypair** (JWK) — run from `infra/api` (where `jose`
   is a dependency; a bare `node -e` from the repo root can't resolve it):
   ```
   node scripts/mint-keys.mjs
   ```
   Put the printed PUBLIC JWK in `wrangler.jsonc` `API_MINT_PUBLIC_JWK`; set the
...
3. **Hyperdrive**: create the binding with SQL response caching disabled, then
   ...
   ```sh
   bunx wrangler hyperdrive create zuse-api-db \
     --connection-string="postgres://…" \
     --caching-disabled
   ```
   API lifecycle transitions require read-after-write consistency; cached
...
6. **Polar sandbox billing**:
   ...
   - Add a webhook endpoint at
     `https://<api-host>/v1/billing/webhook/polar` for subscription created,
...
false, and run the api. Creating `persistent-standard-v1` then issues a
...
- Link proofs are **Ed25519** (asymmetric): the desktop holds the private key and sends
  its public key at link; the api verifies every proof against it. HMAC was rejected —
  the api never sees the desktop's secret, so it can't verify a symmetric signature.
- Migrations run at **deploy** via Drizzle (`bun run db:migrate`), never on Worker cold-start.
- **Managed tunnels**: on link the api creates a per-`(account, environment)` named tunnel,
  ...
  `wss://<hostname>`. Unlink tears the tunnel + DNS down. Chat bytes never touch the api —
