...
import { ApiConfiguration, type ManagedTunnelConfig } from "./config.ts";
import { sha256Hex } from "./crypto.ts";
import { type ApiError, badRequest, forbidden } from "./errors.ts";
 ...
 * with. The api only sets up the route; **no chat bytes ever traverse it** —
 ...
 * api then falls back to the LAN endpoint the desktop advertises.
 */
...
export interface ManagedTunnelProviderApi {
	...
	}) => Effect.Effect<TunnelProvisionResult, ApiError>;
...
export class ManagedTunnelProvider extends Context.Service<
	ManagedTunnelProvider,
	ManagedTunnelProviderApi
>()("@zuse/api/ManagedTunnelProvider") {}
...
const cf = <A>(
	...
	Effect.tryPromise({
		try: async (): Promise<A> => {
			...
			if (allowNotFound && response.status === 404) return undefined as A;
...
> = Layer.effect(
	ManagedTunnelProvider,
	Effect.gen(function* () {
		const { managedTunnel } = yield* ApiConfiguration;

		if (managedTunnel === undefined) {
			return ManagedTunnelProvider.of({
				enabled: false,
				provision: () => Effect.fail(badRequest("managed_tunnel_disabled")),
		...
		const ensureTunnel = (
			tunnelName: string,
		): Effect.Effect<CfTunnel, ApiError> =>
			Effect.gen(function* () {
