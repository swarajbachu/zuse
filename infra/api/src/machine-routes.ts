 ...
 * api-signed proof of which account started this checkout — the completion
...
export const checkoutSuccessUrl = (
	...
	const target = new URL(ApiPaths.billingCheckoutComplete, issuer);
...
const matchBillingWebhookPath = (
	...
	if (path === ApiPaths.billingWebhook) return {};
	const prefix = `${ApiPaths.billingWebhook}/`;
...
const getOwnedMachine = (
	machineId: string,
	accountId: string,
): Effect.Effect<MachinePersistenceRecord, ApiError, MachineStore> =>
	Effect.gen(function* () {
		const store = yield* MachineStore;
...
const enrollmentToken = (request: Request): Effect.Effect<string, ApiError> => {
...
export const routeMachineRequest = (
	request: Request,
): Effect.Effect<Response | null, ApiError, MachineRouteContext> =>
	Effect.gen(function* () {
		...
		if (method === "GET" && path === ApiPaths.billingCheckoutComplete) {
			// Anyone can open this URL, so the page is rendered from the api's own
			...
			const ticket =
				ticketParam === null
					? null
					: yield* parseJwk(apiConfig.mintPublicKey).pipe(
		...
		if (method === "POST" && path === ApiPaths.machineEnroll) {
			...
			const claimed = yield* apiStore.registerEnvironment(
				{
					environmentId: body.environmentId,
					accountId: machine.accountId,
					providerKind: "cloud",
			...
			yield* apiStore.setTunnelAllocation(body.environmentId, {
			...
			if (!completed) {
				if (machine.enrolledEnvironmentPublicKey === undefined) {
					...
					yield* tunnel
						...
						.pipe(Effect.ignore);
					yield* apiStore.deleteEnvironment(
						body.environmentId,
						machine.accountId,
					);
				}
			...
			if (machine.enrolledEnvironmentPublicKey !== undefined) {
				yield* apiStore.revokeEnvironmentCredentials(body.environmentId, nowMs);
			}
			const credentialSecret = yield* randomToken("zenv");
			const credentialId = yield* randomToken("cred", 8);
			yield* apiStore.insertCredential({
				credentialId,
				environmentId: body.environmentId,
				accountId: machine.accountId,
		...
		if (method === "GET" && path === ApiPaths.machineOffers) {
		...
		if (method === "GET" && path === ApiPaths.machines) {
		...
		if (method === "POST" && path === ApiPaths.machines) {
			const principal = yield* requireHostedPrincipal(request);
		...
		if (method === "POST" && destroyMatch !== null) {
			...
			if (machine.environmentId !== undefined) {
				const apiStore = yield* ApiStore;
				yield* apiStore.revokeEnvironmentCredentials(
