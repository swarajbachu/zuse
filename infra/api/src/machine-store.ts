	...
	Effect.gen(function* () {
		...
		return MachineStore.of({
				...
				orDie(
					Effect.gen(function* () {
						...
						const active = yield* sql<{ readonly machine_id: string }>`
							SELECT machine_id FROM api_machines
							WHERE account_id = ${input.accountId}
						...
						const entitlements = yield* sql<EntitlementRow>`
							SELECT entitlement.* FROM api_entitlements entitlement
							WHERE entitlement.account_id = ${input.accountId}
								...
								AND NOT EXISTS (
									SELECT 1 FROM api_machines machine
			...
			getMachine: (machineId) =>
				orDie(
					sql<MachineRow>`
						SELECT * FROM api_machines WHERE machine_id = ${machineId}
					`.pipe(Effect.map((rows) => (rows[0] ? toMachine(rows[0]) : null))),
				),
			findMachineByEnvironmentId: (environmentId) =>
				orDie(
					sql<MachineRow>`
						SELECT * FROM api_machines WHERE environment_id = ${environmentId}
					`.pipe(Effect.map((rows) => (rows[0] ? toMachine(rows[0]) : null))),
				),
			...
			claimDueMachines: (input) =>
				orDie(
					sql<MachineRow>`
						...
						UPDATE api_machines machine SET
