 ...
 * mobile) and extracts the account identity every api record is scoped by.
...
export class WorkosVerifier extends Context.Service<
	WorkosVerifier,
	{
		readonly verify: (
			token: string,
		) => Effect.Effect<WorkosPrincipal, ApiError>;
		readonly exchangeToken: (
			grant: ApiAuthTokenGrant,
		) => Effect.Effect<ApiAuthTokenResponse, ApiError>;
	}
>()("@zuse/api/WorkosVerifier") {}
...
const logWorkosVerifyFailure = (
	...
	console.warn("[zuse-api] WorkOS token verification failed", {
