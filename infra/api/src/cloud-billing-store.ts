...
export const CloudBillingStorePg = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const currentPeriod: CloudBillingStoreApi["currentPeriod"] = (
			...
			sql<PeriodRow>`SELECT * FROM api_cloud_billing_periods WHERE account_id = ${accountId} AND period_start <= ${nowMs} AND period_end > ${nowMs} ORDER BY period_start DESC LIMIT 1`.pipe(
		...
		return CloudBillingStore.of({
			hasProviderEvent: (provider, eventId) =>
				sql<{
					readonly present: boolean;
				}>`SELECT EXISTS(SELECT 1 FROM api_provider_usage_events WHERE provider = ${provider} AND event_id = ${eventId}) AS present`.pipe(
			...
			isProviderEventFinalized: (provider, eventId, providerExecutionId) =>
				...
				}>`SELECT EXISTS(SELECT 1 FROM api_provider_event_finalizations WHERE provider = ${provider} AND (event_id = ${eventId} OR (${providerExecutionId ?? null}::text IS NOT NULL AND provider_execution_id = ${providerExecutionId ?? null}))) AS finalized`.pipe(
			...
			priceWindows: (provider, startedAtMs, endedAtMs) =>
				...
				}>`SELECT version, effective_at, base_nano_usd_per_second, cpu_nano_usd_per_second, memory_nano_usd_per_gib_second FROM api_provider_price_schedule WHERE provider = ${provider} AND effective_at < ${endedAtMs} ORDER BY effective_at ASC`.pipe(
					Effect.map((rows) => {
			...
			periodsOverlapping: (accountId, startedAtMs, endedAtMs) =>
				sql<PeriodRow>`SELECT * FROM api_cloud_billing_periods WHERE account_id = ${accountId} AND period_start < ${endedAtMs} AND period_end > ${startedAtMs} ORDER BY period_start ASC`.pipe(
			...
			setCap: (period, capMicros, nowMs, idempotencyKey) =>
				Effect.gen(function* () {
					yield* sql`SELECT period_id FROM api_cloud_billing_periods WHERE period_id = ${period.periodId} FOR UPDATE`;
					const commandRows = yield* sql<{
						readonly overage_cap_micros: number;
					}>`SELECT overage_cap_micros FROM api_cloud_billing_cap_commands WHERE period_id = ${period.periodId} AND idempotency_key = ${idempotencyKey}`;
					if (commandRows[0] !== undefined) {
						if (Number(commandRows[0].overage_cap_micros) !== capMicros)
							return "idempotency-conflict" as const;
						const rows =
							yield* sql<PeriodRow>`SELECT * FROM api_cloud_billing_periods WHERE period_id = ${period.periodId}`;
						return toPeriod(rows[0] as PeriodRow);
					}
					const incurredRows = yield* sql<{
						readonly total: number;
					}>`SELECT COALESCE(SUM(amount_micros), 0) AS total FROM api_cloud_billing_ledger WHERE period_id = ${period.periodId} AND kind = 'overage-charge'`;
					if (capMicros < Number(incurredRows[0]?.total ?? 0))
						return "below-incurred" as const;
					yield* sql`INSERT INTO api_cloud_billing_cap_commands (period_id, idempotency_key, overage_cap_micros, created_at) VALUES (${period.periodId}, ${idempotencyKey}, ${capMicros}, ${nowMs})`;
					const rows =
						yield* sql<PeriodRow>`UPDATE api_cloud_billing_periods SET overage_cap_micros = ${capMicros}, updated_at = ${nowMs} WHERE period_id = ${period.periodId} RETURNING *`;
					return toPeriod(rows[0] as PeriodRow);
			...
			recordProviderExecutionBatch: (batch) =>
				Effect.gen(function* () {
					...
					const eventRows = yield* sql<{
						readonly event_id: string;
					}>`SELECT event_id FROM api_provider_usage_events WHERE provider = ${batch.provider} AND event_id = ${batch.eventId} FOR UPDATE`;
					if (eventRows[0] === undefined)
						throw new Error("provider usage event missing");
					const finalizedRows = yield* sql<{
						readonly finalized: boolean;
					}>`SELECT EXISTS(SELECT 1 FROM api_provider_event_finalizations WHERE provider = ${batch.provider} AND (event_id = ${batch.eventId} OR (${batch.providerExecutionId ?? null}::text IS NOT NULL AND provider_execution_id = ${batch.providerExecutionId ?? null}))) AS finalized`;
					if (finalizedRows[0]?.finalized === true) return false;
					let metered = false;
					for (const input of batch.usage) {
						const periodRows =
							yield* sql<PeriodRow>`SELECT * FROM api_cloud_billing_periods WHERE period_id = ${input.periodId} FOR UPDATE`;
						...
						FROM api_cloud_billing_ledger WHERE period_id = ${input.periodId}`;
					...
					VALUES (${`ledger:${input.entryId}`}, ${input.periodId}, ${input.accountId}, 'provider-cost', ${input.providerCostMicros}, ${input.entryId}, ${JSON.stringify({ provider: input.provider, resourceKind: input.resourceKind, resourceId: input.resourceId })}, ${input.endedAt}, ${input.nowMs}) ON CONFLICT DO NOTHING`;
						...
						const totalRows = yield* sql<{
							readonly total: number;
						}>`SELECT COALESCE(SUM(amount_micros), 0) AS total FROM api_cloud_billing_ledger WHERE period_id = ${input.periodId} AND kind = 'overage-charge'`;
						const exportedRows = yield* sql<{
							readonly total: number;
						}>`SELECT COALESCE(SUM(amount_cents), 0) AS total FROM api_cloud_billing_outbox WHERE period_id = ${input.periodId}`;
					...
					yield* sql`INSERT INTO api_provider_event_finalizations (provider, event_id, provider_execution_id, finalized_at, expires_at)
			...
			reserveCost: (input) =>
				Effect.gen(function* () {
					const rows =
						yield* sql<PeriodRow>`SELECT * FROM api_cloud_billing_periods WHERE period_id = ${input.periodId} FOR UPDATE`;
					yield* sql`INSERT INTO api_cloud_billing_reservations (period_id, account_id, resource_kind, resource_id, provider, provider_cost_micros, started_at, vcpu_count, memory_mib, expires_at, updated_at)
			...
			acknowledgeOutbox: (outboxId, nowMs) =>
				sql`UPDATE api_cloud_billing_outbox SET acknowledged_at = ${nowMs}, last_error = NULL WHERE outbox_id = ${outboxId}`.pipe(
					Effect.asVoid,
					Effect.orDie,
				),
			retryOutbox: (outboxId, nowMs, error) =>
				sql`UPDATE api_cloud_billing_outbox SET attempt_count = attempt_count + 1, next_attempt_at = ${nowMs} + LEAST(900000, POWER(2, LEAST(attempt_count, 4))::bigint * 60000), last_error = ${error.slice(0, 500)} WHERE outbox_id = ${outboxId}`.pipe(
					Effect.asVoid,
					Effect.orDie,
				),
			...
			recordMeterReconciliation: (input) =>
				sql`INSERT INTO api_cloud_billing_meter_reconciliations
