...
const readJson = <A>(request: Request): Effect.Effect<A, ApiError> =>
	Effect.tryPromise({
...
/** Routes a request to the matching endpoint. Failures surface as ApiError. */
const route = (
	request: Request,
	...
	Effect.gen(function* () {
		...
		if (method === "POST" && path === "/v1/auth/token") {
			...
			const body = yield* Effect.try({
				try: () => Schema.decodeUnknownSync(ApiAuthTokenGrant)(untrustedBody),
		...
		if (method === "POST" && path === "/v1/mobile/devices") {
			const principal = yield* requireDpop(request, API_SCOPES.register);
			const body = yield* readJson<{
		...
		if (method === "POST" && statusMatch !== null) {
			...
			const principal = yield* requireDpop(request, API_SCOPES.status);
		...
		if (method === "POST" && connectMatch !== null) {
			...
			const principal = yield* requireDpop(request, API_SCOPES.connect);
			const environment = yield* store.getEnvironment(environmentId);
			if (
				environment === null ||
				environment.accountId !== principal.accountId
...
export const handleRequest = (
	request: Request,
): Effect.Effect<Response, never, ApiContext> =>
	Effect.gen(function* () {
		const config = yield* ApiConfiguration;
		...
		Effect.catchDefect((defect) =>
			Effect.sync(() => {
				const url = new URL(request.url);
				console.error("[zuse-api] unhandled request defect", {
