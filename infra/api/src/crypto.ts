...
import { type ApiError, badRequest, unauthorized } from "./errors.ts";
...
const importEd25519 = (
	jwk: JWK,
	usage: "verify" | "sign",
): Effect.Effect<CryptoKey, ApiError> =>
	Effect.tryPromise({
 ...
 * private key. The api holds only the public key, so a forged proof (wrong
...
}): Effect.Effect<DpopVerification, ApiError> =>
	Effect.gen(function* () {
		const verified = yield* Effect.tryPromise({
...
/** Mint a short-lived, DPoP-bound access token (JWT, EdDSA-signed by the api). */
...
export interface WorkspaceRuntimeTicketClaims {
	readonly accountId: string;
	readonly workspaceId: string;
	readonly scope: "workspace-runtime";
...
/** Mint a database-independent gateway ticket for the enrolled runtime. */
export const signWorkspaceRuntimeTicket = (input: {
	readonly mintPrivateJwk: JWK;
	readonly issuer: string;
	readonly accountId: string;
	readonly workspaceId: string;
		...
		return yield* Effect.tryPromise({
			try: () =>
				new SignJWT({
					...
					.setProtectedHeader({ alg: "EdDSA", typ: "workspace-runtime+jwt" })
					.setIssuer(input.issuer)
					...
					.setExpirationTime(Math.floor((input.nowMs + input.ttlMs) / 1000))
					.sign(key),
			catch: () => badRequest("workspace_runtime_ticket_sign_failed"),
		});
	});

export const verifyWorkspaceRuntimeTicket = (input: {
	...
	readonly nowMs: number;
}): Effect.Effect<WorkspaceRuntimeTicketClaims, ApiError> =>
	Effect.gen(function* () {
		...
		const verified = yield* Effect.tryPromise({
			try: () =>
				jwtVerify(input.token, key, {
					...
					typ: "workspace-runtime+jwt",
					currentDate: new Date(input.nowMs),
				}),
			catch: () => unauthorized("invalid_workspace_runtime_ticket"),
		});
		const payload = verified.payload;
		if (
			typeof payload.sub !== "string" ||
			payload.workspaceId !== input.expectedWorkspaceId ||
			payload.scope !== "workspace-runtime" ||
			payload.role !== "runtime" ||
			payload.protocol !== input.expectedProtocol ||
			typeof payload.generation !== "number" ||
			typeof payload.gatewayEpoch !== "number"
		)
			return yield* Effect.fail(
				unauthorized("workspace_runtime_ticket_binding_mismatch"),
			);
		return {
			...
			gatewayEpoch: payload.gatewayEpoch,
		};
	});
...
export const verifyWorkspaceClientTicket = (input: {
	...
	readonly nowMs: number;
}): Effect.Effect<WorkspaceClientTicketClaims, ApiError> =>
	Effect.gen(function* () {
		const key = yield* importEd25519(input.mintPublicJwk, "verify");
		const verified = yield* Effect.tryPromise({
			try: () =>
...
export const verifyRuntimeRenewalProof = (input: {
	...
	readonly gatewayEpoch: number;
	readonly nowMs: number;
}): Effect.Effect<RuntimeRenewalProofClaims, ApiError> =>
	Effect.gen(function* () {
...
/** Verify a api-minted access token (for DPoP-protected endpoints). */
...
export const verifyCheckoutReceiptTicket = (input: {
	...
	readonly issuer: string;
	readonly nowMs: number;
}): Effect.Effect<CheckoutReceiptTicketClaims, ApiError> =>
	Effect.gen(function* () {
		const key = yield* importEd25519(input.mintPublicJwk, "verify");
		const verified = yield* Effect.tryPromise({
			try: () =>
