...
Find unfinished work and give your agent the exact context.
...
From the repository root, run `bun install --frozen-lockfile`, `bunx vitest run extensions/code-follow-ups/test`, and `bun run --cwd extensions/code-follow-ups check-types`. Dependencies are pinned in this directory and resolved by the repository's committed `bun.lock`. Official distribution bundles dependencies ahead of time; users do not run a package manager.
