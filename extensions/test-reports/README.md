...
Turn a failed test report into the next agent task.
...
From the repository root, run `bun install --frozen-lockfile`, `bunx vitest run extensions/test-reports/test`, and `bun run --cwd extensions/test-reports check-types`. Dependencies are pinned in this directory and resolved by the repository's committed `bun.lock`. Official distribution bundles dependencies ahead of time; users do not run a package manager.
