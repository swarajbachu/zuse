...
## Check credentials again when moving to cloud

A separate execution environment may need its own agent authentication even when the same provider works locally. Verify model access and repository access independently before starting a long task. A successful checkout does not prove the model can receive a request, and a successful model request does not prove the workspace can push a branch.

Keep model usage separate from cloud compute when reviewing costs. See [pricing](/pricing) for current infrastructure terms and the [open source cloud agents guide](/blog/open-source-cloud-agents) for an end-to-end setup checklist.
