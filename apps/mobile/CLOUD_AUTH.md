...
one mints a short-lived, DPoP-bound connect token from the api and opens the
...
- `src/rpc/api-client.ts` — api HTTP client (dpop-token exchange, list, status, connect).
- `src/store/auth.ts` / `src/store/environments.ts` — auth + discovery state.
- `app/computers.tsx` — the "Your computers" screen.

## Off-network reach
Discovery + connect work on the **same Wi-Fi** today (the api returns the
environment's LAN endpoint). Reaching a computer from cellular needs the managed
**Cloudflare tunnel**, which is deploy-time infra — see the api deploy handoff.
Once the api returns a tunnel `wsBaseUrl`, this client already uses it (via
