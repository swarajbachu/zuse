...
export const sourceOptionsForKind = (
	...
	switch (kind) {
		...
		case "worktree": {
			...
			// The server remains authoritative when it resolves origin/HEAD, so the
			// client must not guess a branch name that may disagree with creation.
			const label = "Default branch";
			return [
				{
