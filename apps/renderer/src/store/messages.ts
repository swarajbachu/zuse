...
/**
 * Live view of one session's durable timeline. Message rows, queue state, and
 * goals live here; visible lifecycle state is projected separately by the
 * session-runtime module so transcript data cannot independently drive global
 * activity indicators.
 */
/**
...
export const checkTimelineLiveness = async (
	...
	try {
		...
		if (throughVersion > appliedVersion) {
			...
			timelineLagObservations.set(sessionId, {
				appliedVersion,
				throughVersion,
			});
			if (
...
export const teardownLiveStreams = async (
	...
	if (sessionId !== undefined) {
		...
		timelineEvictionTimers.set(
			...
			setTimeout(() => {
				...
				useSessionRuntimeStore.getState().releaseTimeline(sessionId);
			}, 5 * 60_000),
		);
		return;
	}
	stopLiveConnectionSubscription();
	const ownedRuntimeSessions = new Set([
		...timelineFibers.keys(),
		...timelineTokens.keys(),
		...timelineStarts,
	]);
	...
	for (const ownedSessionId of ownedRuntimeSessions) {
		useSessionRuntimeStore.getState().releaseTimeline(ownedSessionId);
	}
	await Promise.all(
...
export const useMessagesStore = create<MessagesState>((set, get) => ({
		...
		try {
			if (
				...
				if (cached !== null) {
					...
					useSessionsStore
						.getState()
						.setSessionStatus(sessionId, cached.projection.status);
			...
			const publishTimelineState = (): void => {
				...
				useSessionsStore
					.getState()
					.setSessionStatus(sessionId, projection.status);
				useSessionRuntimeStore
					.getState()
					.observeTimeline(sessionId, projection, timeline.appliedVersion);
				if (projection.status === "idle" || projection.status === "running") {
	...
	send: async (sessionId, input, opts) => {
		...
		try {
			...
			useSessionRuntimeStore
				.getState()
				.settleOptimisticTurn(sessionId, "failed");
			set((s) => ({
				errorBySession: {
					...s.errorBySession,
					[sessionId]: classifyError(err, lookupSessionProvider(sessionId)),
	...
	interrupt: async (sessionId) => {
		...
		try {
			...
			// a dropped terminal frame must never leave tabs or the sidebar spinning.
			useSessionRuntimeStore.getState().settleOptimisticTurn(sessionId, "idle");
			void useSessionsStore.getState().refreshOne(sessionId);
		} catch (err) {
			useSessionRuntimeStore.getState().rollbackOptimisticStop(sessionId);
			set((s) => ({
				errorBySession: {
					...s.errorBySession,
