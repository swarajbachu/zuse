...
const classifyControlPlaneFailure = async (
	...
	if (code !== "not-found" && code !== "conflict") return { kind: "transient" };
	try {
...
const attemptLifetime = () => {
	...
	return {
		...
		dispose: () => {
			if (!controller.signal.aborted) controller.abort(attemptDisposedError());
		},
	};
};
...
class WorkspaceStatusFanout {
	...
	private publish(status: CommandStatus): void {
		for (const subscriber of this.subscribers) {
			if (subscriber.commandId === status.commandId) subscriber.observe(status);
		}
	}
	...
	private async refreshAll(): Promise<void> {
		...
		const statuses = await Promise.all(
			commandIds.map((commandId) => commandStatus(this.workspaceId, commandId)),
		);
				...
				void (async () => {
					...
					while (!settled) {
						try {
							...
							if (settled) return;
							observeFailure(
								await classifyControlPlaneFailure(envelope.workspaceId, cause),
							);
							if (settled) return;
...
const enqueueEnvelope = async (
	...
	try {
		const control = await getControlPlaneRpcClient();
		return await Effect.runPromise(control["cloud.commands.enqueue"](envelope));
	} catch (enqueueCause) {
