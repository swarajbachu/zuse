...
/**
 * Optional resolver that turns a canonical chip token (`@<relPath>`,
 * `[image:<id>]`) back into chip metadata. Supplied by the composer host so
 * that cut/copied chip tokens rehydrate into real chips on paste.
 */
export type ChipTokenResolver = (token: string) => ChipMeta | null;

export const chipResolverFacet = Facet.define<
	ChipTokenResolver,
	ChipTokenResolver | null
>({
	combine: (values) => values[0] ?? null,
});

// `@path` only at start-of-text or after whitespace so emails/handles inside
// prose don't sprout chips; `[image:<id>]` anywhere.
const CHIP_TOKEN_RE = /(?<![^\s])@[^\s]+|\[image:[^\]\s]+\]/g;

/**
 * Rehydrate chips on paste. The doc text keeps canonical tokens, so Cmd+A /
 * Cmd+X / Cmd+V round-trips text fine — this extension additionally re-adds
 * the chip decorations over any pasted tokens the host can resolve.
 */
const chipPasteRehydration = EditorState.transactionExtender.of((tr) => {
	if (!tr.isUserEvent("input.paste")) return null;
	const resolver = tr.startState.facet(chipResolverFacet);
	if (resolver === null) return null;

	const effects: StateEffect<ChipRange>[] = [];
	tr.changes.iterChanges((_fromA, _toA, fromB, _toB, inserted) => {
		const text = inserted.toString();
		for (const match of text.matchAll(CHIP_TOKEN_RE)) {
			const token = match[0];
			const meta = resolver(token);
			if (meta === null) continue;
			const from = fromB + match.index;
			effects.push(addChipEffect.of({ from, to: from + token.length, meta }));
		}
	});
