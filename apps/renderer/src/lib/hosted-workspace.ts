...
export const selectHostedCloudHome = (): void => {
	...
	const shell = getRendererClientBus().snapshot(
		environmentShellResourceKey({ environmentId: EnvironmentId.make("local") }),
	).data;
	useWorkspaceStore.setState({
		folders: shell?.folders ?? [],
		selectedFolderId: shell?.folders[0]?.id ?? null,
		loading: false,
	});
	useChatsStore.setState({ selectedChatId: null });
	useSessionsStore.setState({ selectedSessionId: null });
};
