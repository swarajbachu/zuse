 ...
 * Pierre editor or a side-by-side `@pierre/diffs` patch view. Both
 * bodies stay mounted across toggles so unsaved edits survive
...
// ---------------------------------------------------------------------------
// Pierre edit body — loads a file via fs.readFile and renders the same file
// surface used by review, with native in-place editing from @pierre/diffs/edit.
// ---------------------------------------------------------------------------
...
function PierreEditBody({
	...
	const [pendingComment, setPendingComment] = useState<FileCommentRange | null>(
		null,
	);
	...
	// Mutable per-file working state. Refs keep save/load callbacks stable across
	// keystrokes while Pierre owns rendering, selection and undo history.
	const editorRef = useRef<Editor<FileAnnotationMetadata> | null>(null);
	const docRef = useRef("");
	...
	useEffect(() => {
		const onKeyDown = (event: KeyboardEvent) => {
			if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "s") {
				if (hidden || state.status !== "text") return;
	...
	useEffect(() => {
		if (!matchesRevealedAnnotation || revealedAnnotation === null) return;
		editorRef.current?.focus({
			lineNumber: revealedAnnotation.startLine,
			offset: 72,
		});
	}, [matchesRevealedAnnotation, revealedAnnotation]);
	const annotationRevision = `${visibleAnnotations.map((item) => `${item.id}:${item.comment}`).join("|")}:${pendingComment?.startLine ?? ""}:${pendingComment?.endLine ?? ""}`;
	...
	const lineAnnotations = useMemo<LineAnnotation<FileAnnotationMetadata>[]>(
		() => [
			...visibleAnnotations.map((annotation) => ({
				...
				metadata: { kind: "saved" as const, annotation },
			})),
			...(pendingComment === null
				? []
				: [
	...
	const createPierreEditor = useCallback(
		() =>
			new Editor<FileAnnotationMetadata>({
				persistState: true,
								...
								annotation.metadata?.kind === "draft" ? (
									<DraftReviewAnnotation
										...
										onSave={async (comment) => {
											if (pendingComment === null) return false;
