...
function deriveState(
  ...
  if (providerId === "cursor") {
    if (!a.hasApiKey) return { kind: "signed-out" };
    if (a.apiKeyStatus === "verified") return { kind: "ready", via: "key" };
    return {
      kind: "key-attention",
      invalid: a.apiKeyStatus === "invalid",
    };
  }
  ...
  return (
    ...
    >
      ...
      <span className="flex min-w-0 flex-1 flex-col gap-0.5">
        ...
        <StateLine providerId={providerId} state={state} />
      </span>
      {active ? (
...
function ProviderStatus({
  ...
  const subline =
    state.kind === "signed-out"
      ? providerId === "cursor"
        ? "Paste a User API Key to verify and connect this provider."
        : "Already installed. Just run the login command below."
      : state.kind === "key-attention"
        ? state.invalid
          ? "The saved key was rejected. Replace it with a valid User API Key."
          : "The key is saved, but could not be verified. Recheck when online or replace it."
        : state.kind === "subscription"
  ...
  const command =
    state.kind === "signed-out"
      ? (LOGIN_HINT[providerId] ?? null)
      : state.kind === "key-attention"
        ? null
        : state.kind === "outdated"
          ? (state.command ?? INSTALL_HINT[providerId] ?? null)
          : state.kind === "missing"
            ? (INSTALL_HINT[providerId] ?? null)
            : null;
