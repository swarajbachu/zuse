...
} from "~/lib/code-block-highlight.ts";
import {
	highlightCode,
	readCachedHighlightedCode,
} from "~/lib/highlight-worker.ts";
import { SHIKI_LANGUAGES, SHIKI_THEME } from "~/lib/shiki-config.ts";
 ...
 * for large payloads. While highlighting is pending, the source uses the same
 * pre/code/line geometry as highlighted output so virtual-list measurements
 * and the reader's scroll position remain stable.
 */
export function CodeBlock({
	...
	const html =
		cachedHtml ?? selectCurrentHighlight(highlightResult, highlightKey);
	useEffect(() => {
		if (highlightInput === null || highlightKey === null) {
			setHighlightResult(null);
			return;
		}
		const cached = readCachedHighlightedCode(highlightInput);
		if (cached !== null) {
			setHighlightResult({ key: highlightKey, html: cached });
			return;
		}
		let cancelled = false;
		const timer = window.setTimeout(() => {
			void highlightCode(highlightInput)
				.then((out) => {
					if (!cancelled) setHighlightResult({ key: highlightKey, html: out });
				})
				.catch(() => {
					if (!cancelled) setHighlightResult(null);
				});
		...
		>
			...
			>
				<div
					...
					data-highlighted={html === null ? undefined : true}
					// Shiki escapes source and emits only its token span tree.
