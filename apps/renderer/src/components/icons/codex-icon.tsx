...
/**
 * Codex (OpenAI) brand mark — the official Codex logo. Inlined as a monochrome
 * `currentColor` SVG so it tints with the surrounding text and needs no remote
 * download — same approach as the Grok / Cursor / OpenCode marks. We don't use
 * HugeIcons' `ChatGptIcon` because its filled bulk style doesn't match the real
 * logo.
 */
export function CodexIcon({ className, ...props }: SVGProps<SVGSVGElement>) {
	...
	return (
		<svg
			...
			{...props}
		>
			<title>{uiMessage("chat:codex_icon_codex")}</title>
