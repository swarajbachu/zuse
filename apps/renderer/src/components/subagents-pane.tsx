...
export function SubagentsPane({
		...
		return (
			<div className="flex min-h-0 flex-1 flex-col">
				<div className="flex h-8 shrink-0 items-center gap-1.5 border-b border-border/60 px-2">
					<button
						type="button"
						aria-label={uiMessage("chat:subagents_pane_back_to_subagents")}
	...
	return (
		<div className="min-h-0 flex-1 overflow-y-auto px-2 py-3">
			<AgentSection
				title={uiMessage("chat:subagents_pane_active")}
				groups={active}
				onSelect={handleSelect}
			/>
			<AgentSection
				title={uiMessage("chat:subagents_pane_done", {
					length: String(done.length),
				})}
				groups={done}
				onSelect={handleSelect}
			/>
			{groups.length === 0 ? (
				<p className="px-2 py-8 text-center text-xs text-muted-foreground">
					{uiMessage("chat:subagents_pane_detached_subagents_will_appear_here")}
				</p>
			) : null}
		</div>
	);
		...
		<section className="mb-6">
			...
			{groups.length === 0 ? (
				<p className="px-2 py-2 text-xs text-muted-foreground/70">
					{uiMessage("chat:subagents_pane_no_active_subagents")}
				</p>
			) : (
