...
import { Analytics01Icon } from "@hugeicons-pro/core-bulk-rounded";
import type {
  ProviderId,
  ProviderUsageLimits,
  UsageLimitWindow,
...
function ProviderMenuItem({ providerId }: { providerId: ProviderId }) {
  ...
  const summary =
    provider.windows.find(
      (item) => item.scope === "weekly" || item.scope === "overall",
    ) ?? provider.windows[0];
  const left =
  ...
  return (
    ...
    >
      <MenuSubTrigger>
        ...
        <span>{PROVIDER_DISPLAY[providerId]}</span>
        {summary ? (
...
export function UsageLimitsMenuItems() {
  ...
  return (
    <>
      ...
      <MenuItem onClick={() => openUsage("global")}>
