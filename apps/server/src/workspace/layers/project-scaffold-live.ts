...
export const ProjectScaffoldLive = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const browseDirectory: ProjectScaffold["Service"]["browseDirectory"] = (
			requestedPath,
		) =>
			Effect.gen(function* () {
				const resolved = yield* resolveParent(fs, path, requestedPath);
				...
				if (directory.type !== "Directory") {
					return yield* Effect.fail(
						new WorkspaceInvalidPathError({
							path: resolved,
							reason: "Choose a directory, not a file.",
						}),
					);
				}
				const names = yield* fs.readDirectory(resolved).pipe(
					Effect.mapError(
						(error) =>
							new WorkspaceInvalidPathError({
								path: resolved,
				...
				const rows = yield* Effect.forEach(
					...
					(name) =>
						Effect.gen(function* () {
							...
							return WorkspaceDirectoryEntry.make({
								name,
								path: entryPath,
								kind: stat.value.type === "Directory" ? "directory" : "file",
							});
						}),
					{ concurrency: "unbounded" },
				);
				const entries = rows.filter(
					(row): row is WorkspaceDirectoryEntry => row !== null,
				);
				...
				return WorkspaceDirectoryListing.make({
					path: resolved,
