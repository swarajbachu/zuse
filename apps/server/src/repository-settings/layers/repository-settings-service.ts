...
type MutableRepositorySettingsFile = {
	...
	cloudEnvironmentVariables: Record<string, string>;
...
const parseTomlSettings = (repoPath: string): RepositorySettingsFile => {
	...
	for (const line of fsSync.readFileSync(filePath, "utf8").split(/\r?\n/)) {
		...
		} else if (section === "cloud.environment_variables") {
			settings.cloudEnvironmentVariables[key] = parseTomlString(value);
...
const coerceJsonSettings = (
	...
	const cloudEnv =
		typeof obj.cloudEnvironmentVariables === "object" &&
		obj.cloudEnvironmentVariables !== null
			? (obj.cloudEnvironmentVariables as Record<string, unknown>)
			: null;
	const cloudEnvironmentVariables = { ...fallback.cloudEnvironmentVariables };
	if (cloudEnv !== null)
		for (const [key, val] of Object.entries(cloudEnv))
			if (typeof val === "string") cloudEnvironmentVariables[key] = val;
	return {
		schemaVersion: 1,
...
const writeTomlSettings = (
	...
	const lines = [
		...
		"",
		...tomlStringArray("mcp_disabled_servers", [
		...
		"",
		"[cloud.environment_variables]",
		...Object.entries(settings.cloudEnvironmentVariables)
			.sort(([a], [b]) => a.localeCompare(b))
			.map(([key, value]) => `${key} = ${tomlString(value)}`),
		"",
	];
...
const settingsToFile = (
	...
	cloudEnvironmentVariables: settings.cloudEnvironmentVariables,
...
const rowToFile = (
	...
	return {
		...
		cloudEnvironmentVariables: fallback.cloudEnvironmentVariables,
