...
export class PtyEventJournal {
	...
	constructor(
		...
		private readonly entryBudget = 4_096,
	) {}
	...
	replay(afterSequence?: number): ReplayResult {
		...
		if (
			requestedAfter > this.sequence ||
			(requestedAfter < earliest - 1 && requestedAfter < this.sequence)
		) {
			return {
				_tag: "gap",
				requestedAfter,
	...
	private append(event: PtySequencedEvent, bytes: number): void {
		...
		while (
			(this.retainedBytes > this.byteBudget ||
				this.entries.length > this.entryBudget) &&
