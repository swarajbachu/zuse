...
describe("PTY event journal", () => {
	it("assigns sequences and resumes strictly after a cursor", () => {
		const journal = new PtyEventJournal(1_024, "test-epoch");
		...
		expect(journal.replay(3)).toEqual({ _tag: "events", events: [] });
	});

	it("reports an ahead-of-server cursor instead of silently black-holing future output", () => {
		const journal = new PtyEventJournal(1_024, "test-epoch");
		journal.appendData("a");
		journal.appendData("b");

		expect(journal.replay(99)).toEqual({
			_tag: "gap",
			requestedAfter: 99,
			earliestAvailable: 1,
			latestAvailable: 2,
	...
	it("reports a gap instead of pretending truncated output is complete", () => {
		const journal = new PtyEventJournal(3, "test-epoch");
	...
	it("retains exit metadata without charging it against the byte budget", () => {
		const journal = new PtyEventJournal(1, "test-epoch");
	...
	it("bounds metadata even when output chunks contain no bytes", () => {
		const journal = new PtyEventJournal(1_024, "test-epoch", 2);
		journal.appendData("");
		journal.appendData("");
		journal.appendData("");
