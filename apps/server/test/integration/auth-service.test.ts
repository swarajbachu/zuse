...
const makeHarness = (
	...
	const CredentialsLayer = Layer.succeed(
		...
		CredentialsService.of({
			...
			getSecret: (_account: string) => Effect.succeed(null),
			setSecret: (_account: string, _value: string) => Effect.void,
			removeSecret: (_account: string) => Effect.void,
		}),
	);
	const AuthShellLayer = Layer.succeed(
		...
		AuthShell.of({
			...
			convexRedirectUri: "http://127.0.0.1/convex/callback",
			onConvexCallbackUrl: (_handler: (url: string) => void) => Effect.void,
		}),
	);
